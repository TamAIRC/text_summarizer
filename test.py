...
#     elif os_type == "Linux":
#         # Thêm kiểm tra để xác định là CentOS hay không
#         distro = platform.linux_distribution(full_distribution_name=False)[0]
#         if distro.lower() == "centos":
#             print("Hệ điều hành là CentOS")
#         else:
#             print(f"Hệ điều hành là Linux khác: {distro}")
#     else:
#         print(f"Hệ điều hành không xác định: {os_type}")
...
def assign_levels(lines, config=("Phần", "Chương", "Chapter")):
    def get_level(text):
        ...
        if (
            ...
            or any(text_check.startswith(f"{i}. ") for i in range(1, 10))
        ):
            # print("lv1", text_check)
            return 1

        elif text_check.count(".") == 2 or any(
            text_check.startswith(f"{i}.{j}. ")
            for i in range(1, 10)
            for j in range(1, 10)
        ):
            # print("lv2", text_check)
            return 2

        elif text_check.count(".") == 3 or any(
            text_check.startswith(f"{i}.{j}.{k}. ")
            for i in range(1, 10)
            for j in range(1, 10)
            for k in range(1, 10)
        ):
            # print("lv3", text_check)
            return 3
        else:
            # print("lv0", text_check)
            return 0
